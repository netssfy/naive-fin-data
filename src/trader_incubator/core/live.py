...
class LiveExchange:
    def __init__(
        ...
        # date -> {symbol_key -> close_price}
        self.daily_close_prices: dict[date, dict[str, float]] = {}
        self._last_tick_date: date | None = None
    ...
    def run_tick(self, tick_time: datetime) -> None:
        ...
        tick_date = normalized_tick.date()
        if self._last_tick_date is not None and tick_date != self._last_tick_date and self._current_bars:
            self.daily_close_prices[self._last_tick_date] = {
                sym_key: float(bar.get("close", 0.0) or 0.0)
                for sym_key, bar in self._current_bars.items()
            }
        self._last_tick_date = tick_date
...
def run_season_live(
    ...
    # persist last day's close prices if not yet captured by day-rollover logic
    if exchange._last_tick_date and exchange._current_bars:
        exchange.daily_close_prices.setdefault(
            exchange._last_tick_date,
            {sym_key: float(bar.get("close", 0.0) or 0.0) for sym_key, bar in exchange._current_bars.items()},
        )

    if exchange.daily_close_prices:
        strategies_meta = [
            {"name": s.name, "slug": s.name, "initial_capital": s.initial_capital}
            for s in strategies
        ]
...
def run_all_seasons_live(
    ...
    for season_slug, exchange in exchanges.items():
        # persist last day if not captured by rollover
        if exchange._last_tick_date and exchange._current_bars:
            exchange.daily_close_prices.setdefault(
                exchange._last_tick_date,
                {sym_key: float(bar.get("close", 0.0) or 0.0) for sym_key, bar in exchange._current_bars.items()},
        ...
        if exchange.daily_close_prices:
            season_strategies = [s for s in exchange.strategies]
            strategies_meta = [
                {"name": s.name, "slug": s.name, "initial_capital": s.initial_capital}
                for s in season_strategies
            ]
            persist_backtest_results(
                project_root=Path(project_root),
                season_slug=season_slug,
