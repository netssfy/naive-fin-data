...
class Order:
    ...
    commission: float = 0.0  # 手续费（含滑点），单位：货币
    message: str = ""
...
class SimulatedMatchingEngine:
    ...
    def submit_market_order(
        ...
        trade_value = abs(signed_qty) * price
        commission = round(trade_value * self.fee_rate, 4)
        self.cash_ledger[strategy_name] = self.cash_ledger.get(strategy_name, 0.0) - (signed_qty * price) - commission
